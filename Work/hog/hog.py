...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    onesdigit_player_score = player_score % 10
    tensdigit_opponent_score = opponent_score // 10 % 10
    point = 3 * abs(tensdigit_opponent_score - onesdigit_player_score)
    if point:
        return point
    else:
        return 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    score0 = 0
    score1 = 0
    strategy_fixed = strategy(0, 0)
    while score0 <= goal:
        while score1 <= goal:
            if strategy(score0, score1) != strategy_fixed:
                return False
            score1 += 1
        score0 += 1
        score1 = 0
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    maxave_scoring = 0
    while i <= 10:
        averaged_points = make_averaged(roll_dice, samples_count)(i, dice)
        if averaged_points > maxave_scoring:
            maxave_scoring = averaged_points           
            num_of_dice = i 
        i += 1
    return num_of_dice
    # END PROGRAM 9
...
def final_strategy(score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 12
    if sus_update(0, score, opponent_score) - score >= 10:
        return 0
    else:
