...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    paragraphs = [s for s in paragraphs if select(s)]
    if k >= len(paragraphs):
        return ''
    else:
        return paragraphs[k]
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for str in word_list[1:]:
        now_diff = diff_function(typed_word, str, limit)
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def helper(index, limit):
        if index < 0:
            return 0
        elif limit < 0:
            return limit + 1
        elif typed[index] != source[index]:
            return 1 + helper(index-1, limit-1)
        else:
            return helper(index-1, limit)
   
    result =  helper(min(len(typed)-1, len(source)-1), limit)
...
def report_progress(typed, source, user_id, upload):
    ...
    # method 2
    def get_typedlen(index):
        if index == len(typed) or typed[index] != source[index]:
            return 0
        else:
            return 1 + get_typedlen(index+1)
    progress = get_typedlen(0) / len(source)
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        i_time = timestamps_per_player[i]
        time_per_player += [[i_time[k+1] - i_time[k] for k in range(len(i_time)-1)]]
    return match(words, time_per_player)   
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10 
    # 若使用 [[]] * int,会导致所有元素都指向同一个空列表,修改[0]时就相当于修改全体元素
    result_list = [[] for _ in player_indices]
    for i in word_indices:
        index = 0
        for j in player_indices:
            if get_all_times(match)[index][i] > get_all_times(match)[j][i]: 
                index = j
        result_list[index] += [get_all_words(match)[i]]
    return result_list
    # END PROBLEM 10
