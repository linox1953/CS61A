...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        env.bindings[signature.first] = LambdaProcedure(signature.rest, expressions.rest, env)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    # 若为假或为末项则直接返回当前项
    expr_eval = scheme_eval(expressions.first, env)
    if is_scheme_false(expr_eval) or expressions.rest is nil:
        return expr_eval
...
def do_or_form(expressions, env):
    ...
    # 若为真或为末项则直接返回当前项
    expr_eval = scheme_eval(expressions.first, env)
    if is_scheme_true(expr_eval) or expressions.rest is nil:
        return expr_eval
    # 递归的方法更加简洁
...
def make_let_frame(bindings, env):
    ...
    validate_formals(names) # 注意此函数的说明文档
    # END PROBLEM 14
